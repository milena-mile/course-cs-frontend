class BinaryHeap<T> {
    ...
    constructor(comparator: (a: T, b: T) => number, buffer: Array<T | null> = []) {
        this.buffer = buffer;
    ...
    fromBottom(cursor: number = this.lastIndex) {
        if (this.length() <= 1) {
            return;
        }
...
function heapSort<T>(arr: T[], comparator: (a: T, b: T) => number) {
    const q: BinaryHeap<T> = new BinaryHeap(comparator, arr);
